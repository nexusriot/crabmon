...
pub fn renice(f: &mut Frame<'_>, area: Rect, app: &App) {
    let who = app
        .target
        .as_ref()
        .map(|t| format!("{} ({})", truncate_fit(&t.name, 20), t.pid))
        .unwrap_or_default();
    input_popup(
        f,
        area,
...
pub fn affinity(f: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    input_popup(
        ...
        area,
        app,
        format!(" CPU affinity {who} "),
        "CPU list, e.g. 0-3,8:",
        "[Enter] apply   [Esc] cancel",
    );
}

pub fn detail(f: &mut Frame<'_>, area: Rect, app: &App) {
    let Some(p) = app.selected_row() else { return };
    let inner = frame(
        f,
        area,
        app,
        format!(" {} (pid {}) ", truncate_fit(&p.name, 32), p.pid),
        false,
        (72, 70),
    );
    ...
    let field = |k: &str, v: String| {
        Row::new(vec![
            Cell::from(Span::styled(k.to_string(), Style::default().fg(app.theme.dim))),
    ...
    let table_h = rows.len() as u16;
    let widths = [Constraint::Length(9), Constraint::Min(10)];
    let table_area = Rect { height: table_h.min(inner.height), ..inner };
    f.render_widget(Table::new(rows, widths), table_area);
...
pub fn help(f: &mut Frame<'_>, area: Rect, app: &App) {
    let inner = frame(f, area, app, " Keys ".into(), false, (64, 80));
    let rows = KEYS.iter().map(|(k, v)| {
        Row::new(vec![
...
pub fn alerts(f: &mut Frame<'_>, area: Rect, app: &App) {
    let inner = frame(f, area, app, " Alerts ".into(), false, (64, 60));
    ...
    let rows = app.alerts.rules.iter().map(|r| {
    ...
    let header = Row::new(["Rule", "Kind", "Over", "For", "Target", "State"])
...
pub fn audit_log(f: &mut Frame<'_>, area: Rect, app: &App) {
    let inner = frame(f, area, app, " Action log ".into(), false, (76, 70));
    ...
    let rows = app.audit_entries.iter().take(inner.height as usize).map(|line| {
