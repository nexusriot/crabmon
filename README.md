...
- **Processes** — PID, user, state, CPU%, a CPU sparkline, RSS, virtual size,
  thread count, nice, disk IO, run time and command, sortable on every column
  (keyboard or by clicking the header), with a tree view, a query language, and a
  detail pane on `Enter` showing the process's open sockets. Threads are hidden
  by default (`H` shows them), so the list reflects processes rather than every
  kernel task. Tag several with `Space` to signal, renice or pin them at once.
...
- **Grouping** — aggregate the process table by systemd unit, container or user,
  totalling CPU, memory and disk IO per group.
- **Alerts** — configurable thresholds with a hold time and an optional command
  hook.
- **Export** — write a snapshot as JSON or CSV from the TUI, or run
  `crabmon --once` headless.
- **Record & replay** — `--record` appends every sample to a JSONL file (trimmed
...
- **Prometheus** — `--serve :9100` runs headless and exposes every panel's
  metrics, plus the busiest processes (capped, and configurable).
- **Remote** — `--remote host` monitors another machine over SSH, with no
  daemon and no open port.
...
| `-a`, `--ascending` | Sort ascending (default is descending) |
...
| `--serve <ADDR>` | Serve Prometheus metrics on ADDR, headless |
...
`--record` writes one JSON object per sample. Frames are trimmed to the busiest
100 processes with argv truncated to 200 characters, which is roughly 55 KiB a
frame — about 240 MiB for an hour at the default interval. `[record]` controls
all three limits.
...
sort_by = "cpu"             # pid|name|cpu|mem|virt|disk|time|user|state|threads
sort_desc = true
...
- `--remote` needs crabmon installed on the far end and non-interactive SSH
  (`BatchMode=yes`); the refresh rate is bounded by the SSH round trip.
